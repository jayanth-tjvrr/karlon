...
const ProfilesAnnotationKey = "kkarlon.io/profiles"
...
func List(config *restclient.Config, ns string) (apslist []argoappv1.ApplicationSet, err error) {
	...
	req, err := labels.NewRequirement("kkarlon-type", selection.In, []string{"application"})
	if err != nil {
...
func Delete(config *restclient.Config, ns string, name string) error {
	...
	if app.Labels["kkarlon-type"] != "application" {
		return fmt.Errorf("applicationset %s is not an kkarlon app", name)
	}
