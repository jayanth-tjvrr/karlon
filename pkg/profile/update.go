...
func Update(
	...
	existingBundles, err := bundle.List(config, kkarlonNs)
	if err != nil {
	...
	prof, err := GetAugmented(config, profileName, kkarlonNs)
	if err != nil {
	...
	if prof.Spec.RepoUrl != "" {
		...
		bndl, err := bundle.GetBundlesFromProfile(&prof.Profile, corev1, kkarlonNs)
		if err != nil {
