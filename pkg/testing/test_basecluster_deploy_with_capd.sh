...
repodir=/tmp/karlon-testbed-git-clone/myrepo
...
if karlon basecluster validategit --repo-url ${repourl} --repo-path ${repopath} 2> /tmp/stdout.txt; then
    ...
    if ! karlon basecluster preparegit --repo-url ${repourl} --repo-path ${repopath}; then
        echo preparegit failed
        exit 1
    fi
fi

karlon cluster create --cluster-name capd-1 --repo-url http://172.17.0.1:8188/myrepo.git --repo-path baseclusters/capd-test --profile dynamic-2-calico
