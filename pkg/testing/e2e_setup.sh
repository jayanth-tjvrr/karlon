...
# Setting os for karlon binary download
...
tb_cntr_name='karlon-e2e-testbed'
...
if ! which karlon &>/dev/null; then
  make build
  sudo ln -s "$(pwd)/bin/karlon" /usr/local/bin/karlon
fi

echo "${workspace_repo_url}"
global_workspace_repo_url="http://${bridge_addr}:${git_server_port}/${GIT_USER}/myrepo.git"
wait_until "karlon git register ${global_workspace_repo_url} --user ${GIT_USER} --password ${GIT_PASSWORD} --alias default" 2 30
...
e2eTemplate=${karlon_repo}/testing/capi-quickstart-e2e-template.yaml
manifestfile=${karlon_repo}/testing/capi-quickstart-e2e-test.yaml
...
if ! karlon bundle list | grep xenial >/dev/null; then
  echo "Adding xenial manifests"
  pushd ${workspace_repo}
  mkdir -p bundles/xenial
  cp "${karlon_repo}/examples/bundles/xenial.yaml" bundles/xenial
...
if ! karlon bundle list | grep guestbook >/dev/null; then
...
mv "${karlon_repo}/testing/${manifestfile}" "${baseclusterdir}/${manifestfile}"
