...
eksConfig=${karlon_repo}/testing/eksctl_config.yaml
envsubst < $eksConfig > ${karlon_repo}/testing/eksctl.yaml
eksctl create cluster --config-file ${karlon_repo}/testing/eksctl.yaml
...
if ! which karlon &>/dev/null; then
  make build
  sudo ln -s -f "$(pwd)/bin/karlon" /usr/local/bin/karlon
fi

karlon init --username "${GIT_USER}" --password "${gittea_token}" --repoUrl "${workspace_repo_url}" -e -y
...
clusterctl generate cluster capi-quickstart --flavor eks \
  ...
  --infrastructure aws \
  >${karlon_repo}/testing/capi-quickstart-cas-e2e-test.yaml
