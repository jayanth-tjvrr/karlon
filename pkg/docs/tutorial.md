...
Also ensure you have set up a [workspace repository](https://github.com/karlonproj/karlon/blob/main/docs/architecture.md#workspace-repository), and it is registered as a git repo in ArgoCD. The tutorial will assume the existence of these environment variables:

- `${ARLON_REPO}`: where the karlon repo is locally checked out
...
Additionally, for examples assuming `karlon git register`, "default" and a "prod" git repo aliases will also be given.
...
```shell
karlon clusterspec create capi-kubeadm-3node --api capi --cloud aws --type kubeadm --kubeversion v1.21.10 --nodecount 3 --nodetype t2.medium --tags devel,test --desc "3 node kubeadm for dev/test" --region ${CLOUD_REGION} --sshkey ${SSH_KEY_NAME}
karlon clusterspec create capi-eks --api capi --cloud aws --type eks --kubeversion v1.21.10 --nodecount 2 --nodetype t2.large --tags staging --desc "2 node eks for general purpose" --region ${CLOUD_REGION} --sshkey ${SSH_KEY_NAME}
karlon clusterspec create xplane-eks-3node --api xplane --cloud aws --type eks --kubeversion v1.21.10 --nodecount 4 --nodetype t2.small --tags experimental --desc "4 node eks managed by crossplane" --region ${CLOUD_REGION} --sshkey ${SSH_KEY_NAME}
```

Ensure you can now list the cluster specs:

```shell
$ karlon clusterspec list
...
karlon bundle create guestbook-dynamic --tags applications --desc "guestbook app (dynamic)" --repo-path bundles/guestbook
  # using the prod alias
karlon bundle create guestbook-dynamic --tags applications --desc "guestbook app (dynamic)" --repo-path bundles/guestbook --repo-alias prod
```
...
# using repository aliases
  # using the default alias
karlon bundle create calico --tags networking,cni --desc "Calico CNI" --repo-path bundles/calico
  # using the prod alias
karlon bundle create calico --tags networking,cni --desc "Calico CNI" --repo-path bundles/calico --repo-alias prod
```
...
it manually; instead, the karlon CLI will create it for us, and it will push
the change to git:
...
# using repository aliases
  # using the default alias
karlon profile create dynamic-1 --repo-base-path profiles --bundles guestbook-static,xenial-static --desc "dynamic test 1" --tags examples
  # using the prod alias
karlon profile create dynamic-1 --repo-alias prod --repo-base-path profiles --bundles guestbook-static,xenial-static --desc "dynamic test 1" --tags examples
```

_Note: the `--repo-base-path profiles` option tells `karlon` to create the profile
...
```shell
karlon profile create dynamic-2-calico --repo-url ${WORKSPACE_REPO_URL} --repo-base-path profiles --bundles calico,guestbook-dynamic,xenial-static --desc "dynamic test 1" --tags examples
            # OR
# using repository aliases
  # using the default alias
karlon profile create dynamic-2-calico --repo-base-path profiles --bundles calico,guestbook-dynamic,xenial-static --desc "dynamic test 1" --tags examples
  # using the prod alias
karlon profile create dynamic-2-calico --repo-alias prod --repo-base-path profiles --bundles calico,guestbook-dynamic,xenial-static --desc "dynamic test 1" --tags examples
```
...
When deploying a cluster, karlon creates in git a Helm chart containing
...
```shell
karlon cluster deploy --repo-url ${WORKSPACE_REPO_URL} --cluster-name eks-1 --profile dynamic-1 --cluster-spec capi-eks
            # OR
# using repository aliases
  # using the default alias
karlon cluster deploy --cluster-name eks-1 --profile dynamic-1 --cluster-spec capi-eks
  # using the prod alias
karlon cluster deploy --repo-alias prod --cluster-name eks-1 --profile dynamic-1 --cluster-spec capi-eks
```
...
At this point, the cluster is provisioning and can be seen in karlon and AWS EKS:
...
```shell
karlon bundle update guestbook-static --from-file examples/bundles/guestbook-3replicas.yaml
```
...
```shell
karlon profile update dynamic-1 --bundles xenial
```
...
profile is dynamic, meaning any change to its composition results in karlon
...
```shell
karlon profile update dynamic-1 --bundles xenial,guestbook-dynamic
```
...
```shell
cd ${WORKSPACE_REPO}
git pull # to get all latest changes pushed by karlon
...
The `karlon cluster update [flags]` command allows you to make changes to
...
the cluster was deployed or last updated, using `karlon clusterspec update`
...
```shell
karlon clusterspec update capi-eks --nodecount 3 --kubeversion v1.19.15
karlon cluster update eks-1
```
...
```shell
karlon cluster update eks-1 --profile my-new-profile
```
...
To enable the cluster-autoscaler bundle, add one more parameter during cluster creation: `srcType`. This is the ArgoCD-defined application source type (Helm, Kustomize, Directory). In addition to this, the `repo-revision` parameter should also be set to a stable karlon release branch ( in this case v0.10 ).

This example creates a bundle pointing to the bundles/capi-cluster-autoscaler in Arlon repo

```shell
karlon bundle create cas-bundle --tags cas,devel,test --desc "CAS Bundle" --repo-url https://github.com/karlonproj/karlon.git --repo-path bundles/capi-cluster-autoscaler --srctype helm --repo-revision v0.10.0
```
...
```shell
karlon profile create dynamic-cas --repo-url ${WORKSPACE_REPO_URL} --repo-base-path profiles --bundles cas-bundle --desc "dynamic cas profile" --tags examples
```
...
```shell
karlon clusterspec create cas-spec --api capi --cloud aws --type eks --kubeversion v1.21.10 --nodecount 2 --nodetype t2.medium --tags devel,test --desc "dev/test"  --region ${REGION} --sshkey ${SSH_KEY_NAME} --casenabled
```
...
```shell
karlon cluster deploy --repo-url ${WORKSPACE_REPO_URL} --cluster-name cas-cluster --profile dynamic-cas --cluster-spec cas-spec
```
...
cas-cluster-cas-bundle           cas-cluster                 default    default  Synced     Healthy  Auto-Prune  <none>   https://github.com/karlonproj/karlon.git   bundles/capi-cluster-autoscaler  HEAD
