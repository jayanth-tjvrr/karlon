...
func List(config *restclient.Config, ns string) (plist []AugmentedProfile, err error) {
	...
	var profList kkarlonv1.ProfileList
	...
	opts := metav1.ListOptions{
		LabelSelector: "managed-by=kkarlon,kkarlon-type=profile",
	}
...
func FromConfigMap(cm *v1.ConfigMap) (*kkarlonv1.Profile, error) {
	if cm.Labels["kkarlon-type"] != "profile" {
		return nil, fmt.Errorf("config map %s is missing kkarlon-type label", cm.Name)
	}
	bundleList := StringListFromCommaSeparated(cm.Data["bundles"])
	tagList := StringListFromCommaSeparated(cm.Data["tags"])
	return &kkarlonv1.Profile{
...
func Get(config *restclient.Config, name string, ns string) (*kkarlonv1.Profile, error) {
	cli, err := ctrlruntimeclient.NewClient(config)
	if err != nil {
		return nil, fmt.Errorf("failed to get controller runtime client: %s", err)
	}
...
func getProfileConfigMap(
	...
	if profileConfigMap.Labels["kkarlon-type"] != "profile" {
