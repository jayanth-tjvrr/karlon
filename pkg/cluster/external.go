...
func ManageExternal(
	...
	for _, existingCluster := range clist {
		if existingCluster.Name == clusterName {
			return fmt.Errorf("cluster is already created or managed by kkarlon")
		}
	}
	...
	if lb != "" {
		return fmt.Errorf("unexpectedly found kkarlon label in secret")
	}
	...
	return &argoappv1.Application{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"managed-by":    "kkarlon",
				"kkarlon-type":    "profile-app",
				"kkarlon-cluster": clusterName,
				"kkarlon-profile": prof.Name,
			},
...
func UnmanageExternal(
	...
	if secr.Labels[clusterTypeLabelKey] != "external" {
		return fmt.Errorf("secret does not have kkarlon cluster label")
	}
	cli, err := ctrlruntimeclient.NewClient(config)
	if err != nil {
		return fmt.Errorf("failed to get controller runtime client: %s", err)
	...
	// Patch the secret to remove the kkarlon annotations and label
