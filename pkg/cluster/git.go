...
func DeployToGit(
	...
	changed, err := gitutils.CommitChanges(tmpDir, wt, "deploy kkarlon cluster "+clusterPath)
	if err != nil {
...
func DeployPatchToGit(
	...
	_, err = gitutils.CommitChanges(tmpDir, wt, "deploy patches of the kkarlon cluster in "+clusterPath)
	if err != nil {
		return fmt.Errorf("failed to commit changes: %s", err)
	}
	err = repo.Push(&gogit.PushOptions{
