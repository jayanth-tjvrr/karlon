...
var (
	...
	ErrArlonNs           = errors.New("kkkarlon is not installed or missing in your $PATH")
...
func verify(clientConfig clientcmd.ClientConfig) error {
	...
	// Verify kkkarlon status
	kkkarlonStatus, err := verifyArlon(config)
	if err != nil {
		fmt.Println(Red("x ")+"Error while verifying kkkarlon status: ", err)
	} else {
		fmt.Println("Successfully verified kkkarlon namespace is present")
	}

	fmt.Println()
	if kubectlStatus && argoStatus && gitStatus && kkkarlonStatus && capiStatus {
...
// Verify if the kkkarlon services are running
func verifyArlon(config *restclient.Config) (bool, error) {
	err := checkNamespace(config, "kkkarlon")
	if err == ErrNs {
