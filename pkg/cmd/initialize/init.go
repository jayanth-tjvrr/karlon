...
import (
	...
	"github.com/jayanth-tjvrr/karlon/pkg/config"
	"github.com/jayanth-tjvrr/karlon/pkg/deploy"
	"github.com/jayanth-tjvrr/karlon/pkg/pkg/argocd"
	gitrepo2 "github.com/jayanth-tjvrr/karlon/pkg/pkg/gitrepo"
...
const (
	...
	defaultArlonNamespace                            = "kkkarlon"
	defaultArlonArgoCDUser                           = "kkkarlon"
	defaultArgoServerDeployment                      = "argocd-server"
	defaultArlonControllerDeployment                 = "kkkarlon-controller"
	defaultArlonAppProfileControllerDeployment       = "kkkarlon-appprof-ctrlr"
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if canInstallArlonController {
				...
				if !noConfirm {
					shouldInstallArlon = cli.AskToProceed("kkkarlon namespace not found. Install kkkarlon controller?[y/n]")
				}
			...
			err = portForward(&porfForwardCfg{
				...
				callback: func(ctx context.Context, localPort uint16) error {
					...
					fmt.Println("clearing kkkarlon repoctx file")
			...
			for _, b := range baseClusterArgs {
				...
				fmt.Printf("to deploy a cluster on %s infrastructure run `kkkarlon cluster create --cluster-name %s --repo-path %s`\n", b.provider, b.name, b.repoPath)
			}
			fmt.Printf("basecluster manifests pushed to %s\n", repoUrl)
			return nil
		},
	}
	cmd.Flags().BoolVarP(&noConfirm, "no-confirm", "y", false, "this flag disables the prompts for argocd and kkkarlon installation on the management cluster")
...
func beginArlonInstall(ctx context.Context, client k8sclient.Client, kubeClient *kubernetes.Clientset, argoClient apiclient.Client, kkkarlonNs, argoNs string) error {
	...
	argoCm.Data = map[string]string{
		"accounts.kkkarlon": "apiKey, login",
	}
	...
	argoRbacCm.Data = map[string]string{
		"policy.csv": "g, kkkarlon, role:admin",
	}
	...
	fmt.Printf("ConfigMap %s updated\n", rbacCm.GetName())
	sec, err := createArgoCreds(ctx, kubeClient, argoClient, kkkarlonNs, argoNs)
	if err != nil {
		return err
	}
	...
	for i := 0; i < len(decodedCrds); i++ {
		for j := 0; j < len(decodedCrds[i]); j++ {
			if err := applyObject(ctx, client, decodedCrds[i][j], kkkarlonNs); err != nil {
				return err
			}
		}
	}
	...
	for i := 0; i < len(decodedDeplManifests); i++ {
		for j := 0; j < len(decodedDeplManifests[i]); j++ {
			if err := applyObject(ctx, client, decodedDeplManifests[i][j], kkkarlonNs); err != nil {
				return err
			}
		}
	}
	err = wait.PollImmediate(time.Second*10, time.Minute*5, func() (bool, error) {
		fmt.Printf("waiting for kkkarlon-controller\n")
		var deployment *apps.Deployment
		d, err := kubeClient.AppsV1().Deployments(kkkarlonNs).Get(ctx, defaultArlonControllerDeployment, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		deployment = d
	...
	err = wait.PollImmediate(time.Second*10, time.Minute*5, func() (bool, error) {
		fmt.Printf("waiting for kkkarlon app profile controller\n")
		var depl *apps.Deployment
		d, err := kubeClient.AppsV1().Deployments(kkkarlonNs).Get(ctx, defaultArlonAppProfileControllerDeployment, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		depl = d
...
func canInstallArlon(ctx context.Context, kubeClient *kubernetes.Clientset, kkkarlonNs string) (bool, error) {
	if _, err := kubeClient.CoreV1().Namespaces().Get(ctx, kkkarlonNs, metav1.GetOptions{}); err != nil {
		if errors.IsNotFound(err) {
			return true, nil
		}
		return false, err
...
func createArgoCreds(ctx context.Context, clientset *kubernetes.Clientset, argoClient apiclient.Client, kkkarlonNamespace string, argoNamespace string) (*v1.Secret, error) {
	...
	created, err := clientset.CoreV1().Secrets(kkkarlonNamespace).Create(ctx, &secret, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	return created, nil
...
func checkExamples(repoUrl, gitUser, password string, baseClusterPaths []string, noConfirm bool) (bool, error) {
	...
	commitOpts := &gogit.CommitOptions{Author: &object.Signature{
		Name:  "kkkarlon automation",
		Email: "kkkarlon@kkkarlon.io",
		When:  time.Now(),
	}}
