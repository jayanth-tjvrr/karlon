...
func NewCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Example: `kkkarlon install --kubectlPath <string> --argocdPath <string> --kubeconfigPath /path/to/kubeconfig # install CAPI core provider and CLI tools, with the given argocd path and kubeconfig path
				  kkkarlon install # installs CLI tools and CAPI
				  kkkarlon install --tools-only # installs only the CLI tools
				  kkkarlon install --capi-only # installs only CAPI
				  kkkarlon install --capi-only --infrastructure aws # installs CAPA provider on the management cluster
		`,
